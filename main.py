...
# ========= 在导入 pdfplumber 之前，优先解决 pypdfium2 / pdfium.dll 路径问题 =========
# 兼容多种打包方式（PyInstaller onefile / onedir）以及不同版本的 pypdfium2

def _ensure_pdfium_dll():
    """
    尝试在打包后的目录或临时解压目录中寻找 pdfium.dll，
    找到后通过 PATH / add_dll_directory / 环境变量 告诉 pypdfium2 使用该路径。
    """
    # 1. 基础目录：开发环境用源码目录，打包环境用 _MEIPASS
    if getattr(sys, "frozen", False):
        base_path = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(sys.argv[0])))
    else:
        base_path = os.path.dirname(os.path.abspath(__file__))

    # 2. exe 所在目录（PyInstaller onedir 时资源通常在这里）
    exe_dir = os.path.dirname(os.path.abspath(sys.argv[0]))

    candidate_dirs = [
        # 老版本 pypdfium2_raw 目录
    ...
    # 如果还没找到，再在 base_path / exe_dir 里递归搜索一次（防止目录名有版本号之类）
    ...
    if not pdfium_dir:
        # 找不到就直接返回，让后续逻辑走 pdfplumber 默认后端（可能退回到纯文本解析）
        return
    ...
    # 兼容旧版 pypdfium2_raw 直接在 sys.path 中查找的情况
...
def detect_tesseract_path():
    ...
    1. 配置文件中的路径（优先级最高）
    2. 环境变量 TESSERACT_PATH
    3. 程序目录下 Tesseract-OCR\\tesseract.exe（适合随程序一起打包）
    4. 常见安装路径（Program Files 等）
    5. 系统 PATH 中是否存在 tesseract
    找不到则返回 None
    """
    # 1. 配置文件中的路径（优先级最高）
    config = load_config()
    ...
    # 2. 程序目录内的相对路径（兼容 PyInstaller 打包后的 _MEIPASS）
...
def init_tesseract_or_warn(parent=None) -> bool:
    """
    初始化 Tesseract：
    - 能找到时：设置 pytesseract 路径和 TESSDATA_PREFIX，返回 True
    - 找不到时：弹出提示并返回 False
    """
    ...
    if tesseract_path:
        ...
        QMessageBox.warning(
            parent,
            "Tesseract 未找到",
            "未能自动找到 Tesseract-OCR 程序。\n\n"
            "请先安装 Tesseract-OCR，并将其路径写入环境变量 TESSERACT_PATH，\n"
            "或与本软件同级目录放置 Tesseract-OCR 文件夹。"
        )
        return False
...
class InvoiceParser:
    ...
    def parse_invoice(self, pdf_path):
        ...
        # 同时从文本中解析“价税合计（小写）”作为含税总金额
        text_total = self.extract_total_amount(text)

        # 优先使用文本中的价税合计（含税金额），没有时再使用明细金额之和
    ...
    def validate_invoice(self, invoice, all_invoices=None):
        ...
        if not invoice.get('seller') or invoice.get('seller', '').strip() == '':
            warnings.append('销售方为空')
        
        buyer = (invoice.get('buyer') or '').strip()
        if not buyer:
            warnings.append('购买方为空')
        else:
            # 购买方抬头校验：与配置中的 buyer_expected_name 对比
            cfg = load_config()
            expected_buyer = (cfg.get('buyer_expected_name') or '苏州大学').strip()
            if expected_buyer and buyer != expected_buyer:
                # 抬头不一致按“错误”处理，便于筛查
                errors.append(f'购买方抬头不匹配，当前为“{buyer}”，期望为“{expected_buyer}”')
        ...
        if items:
            ...
            # 允许一定的误差（0.01元），因为可能有四舍五入
            if abs(items_total - total_amount) > 0.01:
                if abs(items_total - total_amount) > 1.0:
                    errors.append(f'明细金额总和（¥{items_total:.2f}）与总金额（¥{total_amount:.2f}）不一致，差异：¥{abs(items_total - total_amount):.2f}')
                else:
                    warnings.append(f'明细金额总和（¥{items_total:.2f}）与总金额（¥{total_amount:.2f}）有轻微差异：¥{abs(items_total - total_amount):.2f}')
        else:
            warnings.append('没有商品明细')
        
        # 3. 发票号重复检测（如果提供了所有发票列表）
        if all_invoices is not None:
            ...
            if invoice_number:
                ...
                if duplicate_count > 0:
                    errors.append(f'发票号重复（发现 {duplicate_count} 张相同发票号的发票）')
        
        # 4. 文件路径检查
        file_path = invoice.get('file_path', '')
        if not file_path:
            errors.append('文件路径为空')
        ...
        for idx, item in enumerate(items, 1):
            if not item.get('name') or not item.get('name', '').strip():
                warnings.append(f'第 {idx} 项商品名称为空')
            ...
            try:
                ...
                if quantity <= 0:
                    warnings.append(f'第 {idx} 项商品数量无效（≤0）')
                if price < 0:
                    warnings.append(f'第 {idx} 项商品单价为负')
                if amount < 0:
                    warnings.append(f'第 {idx} 项商品金额为负')
                
                # 检查数量×单价是否等于金额（允许小误差）
                expected_amount = quantity * price
                if abs(expected_amount - amount) > 0.01:
                    warnings.append(f'第 {idx} 项商品：数量×单价（{quantity}×{price:.2f}={expected_amount:.2f}）与金额（{amount:.2f}）不一致')
            except (ValueError, TypeError):
                warnings.append(f'第 {idx} 项商品数值格式错误')
    ...
    def extract_items_alternative(self, text):
        ...
        # 精确匹配电子发票的商品行格式
        # 格式：项目名称 规格型号 单位 数量 单价 金额 税率 税额
        lines = text.split('\n')
        ...
        for line in lines:
            ...
            # 查找包含*的项目行，但排除表头和汇总行
            if '*' in line and not any(keyword in line for keyword in ['合计', '价税合计']):
                # 分割行内容
                parts = re.split(r'\s+', line)

                # 我们不再强依赖固定位置，而是：
                # 1. 找到第一个包含“%”的字段（税率）
                # 2. 在税率之前，最后一个数字视为“金额”，倒数第二个数字视为“单价”，再往前一个视为“数量”（如果存在）
                try:
                    percent_idx = None
                    ...
                    # 单价：金额前一个数字（如果有），否则与金额相同。
                    # 对于只有两个数字且第一个明显异常（如 1202.65 与 202.65），
                    # 直接认为单价等于金额，前面的数字视为噪声。
                    ...
                    # 数量：再往前一个数字（如果有），否则根据 金额/单价 估算或默认为 1
                    ...
                    # 如果是折扣行（金额为负），不希望出现负单价：
                    # - 单价设为金额的绝对值
                    # - 数量固定为 1（表示一笔折扣）
                    # 这样在界面中看到的是“单价为正，金额为负”的折扣行，更符合直觉。
                    if amount_with_tax < 0:
                        price_with_tax = abs(amount_with_tax)
                        quantity = 1.0

                    # 单位：第一个数字前一个字段通常是单位，例如 “… 个 3 2.27 6.83 …”
                    ...
                    # 折扣行在名称后面加上“（折扣）”以便识别
                    if amount_with_tax < 0 or '优惠' in line:
                        product_name = (product_name + '（折扣）').strip()
    ...
    def parse_item_line(self, line):
        """解析商品行"""
        # 使用正则表达式分割可能的商品信息
        # 匹配模式：项目名称 规格型号 单位 数量 单价 金额 税率 税额
    ...
    def extract_total_amount(self, text):
        """提取总金额"""
        # 这里只取“价税合计（小写）”对应的含税金额，避免误抓到前面的“合计（未税金额）”
        patterns = [
            r'价税合计.*?[（(]小写[）)]\s*¥?\s*(\d+\.?\d*)',
            r'价税合计[：:]\s*¥?\s*(\d+\.?\d*)',
        ]
    ...
    def run(self):
        ...
        for i, pdf_file in enumerate(self.pdf_files):
            try:
                ...
                self.error.emit(f"处理文件 {pdf_file} 时出错: {str(e)}")
...
class PurchaseRecordMatcher(QThread):
    ...
    def _extract_paid_amount_from_image(self, img_path):
        """从图片中识别实付金额，仅用于线程内部调用"""
        try:
            image = Image.open(img_path)
        except Exception as e:
            print(f"OCR 打开图片失败 {img_path}: {e}")
            return None
        ...
        keywords = ['实付', '实收', '实付金额', '支付金额', '应付金额', '合计', '总计']
        candidate_amounts = []
...
class InvoicePreviewDialog(QDialog):
    ...
    def zoom_in(self):
        ...
        if self.original_pixmap:
            self.zoom_factor = min(self.zoom_factor * 1.2, 5.0)  # 最大5倍
            self._update_display()
    
    def zoom_out(self):
        """缩小"""
        if self.original_pixmap:
            self.zoom_factor = max(self.zoom_factor / 1.2, 0.2)  # 最小0.2倍
            self._update_display()
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # ========== Tesseract OCR 路径设置 ==========
        tesseract_group = QGroupBox('Tesseract OCR 配置')
        tesseract_layout = QVBoxLayout(tesseract_group)
        
        desc_label = QLabel('请选择 Tesseract-OCR 的可执行文件路径（tesseract.exe）：')
        desc_label.setWordWrap(True)
        tesseract_layout.addWidget(desc_label)
        
        path_layout = QHBoxLayout()
        self.tesseract_path_edit = QLineEdit()
        self.tesseract_path_edit.setPlaceholderText('例如：C:\\Program Files\\Tesseract-OCR\\tesseract.exe')
        ...
        # ========== OCR / 匹配行为设置 ==========
        behavior_group = QGroupBox('识别与匹配行为')
        ...
        self.ocr_lang_combo.addItem('自动（不指定，默认识别）', '')
        self.ocr_lang_combo.addItem('中文 + 英文（chi_sim+eng）', 'chi_sim+eng')
        self.ocr_lang_combo.addItem('仅中文（chi_sim）', 'chi_sim')
        self.ocr_lang_combo.addItem('仅英文（eng）', 'eng')
        ...
        # 启动时自动加载上次发票
        self.auto_load_checkbox = QCheckBox('启动时自动加载上次保存的发票数据')
        ...
        self.match_tol_spin.setSuffix(' 元')
        ...
        backup_interval_label = QLabel('备份间隔（天）：')
        ...
        backup_path_label = QLabel('默认备份路径：')
        self.backup_path_edit = QLineEdit()
        self.backup_path_edit.setPlaceholderText('留空则使用程序目录')
        ...
        self.export_path_edit.setPlaceholderText('留空则每次询问')
        ...
        # 导出时包含验证信息
        self.include_validation_checkbox = QCheckBox('导出时包含验证信息')
        ...
        # 导入时自动验证
        self.auto_validate_checkbox = QCheckBox('导入发票时自动验证')
        ...
        strict_label = QLabel('验证严格程度：')
        self.validation_strict_combo = QComboBox()
        self.validation_strict_combo.addItem('宽松（仅检查严重错误）', 'loose')
        self.validation_strict_combo.addItem('标准（检查错误和重要警告）', 'normal')
        self.validation_strict_combo.addItem('严格（检查所有问题）', 'strict')
        ...
        # ========== 抬头设置 ==========
        ui_group = QGroupBox('抬头/界面设置')
        ui_layout = QVBoxLayout(ui_group)
        
        # 发票抬头校验名称
        buyer_layout = QHBoxLayout()
        buyer_label = QLabel('期望购买方抬头：')
        self.buyer_expected_edit = QLineEdit()
        self.buyer_expected_edit.setPlaceholderText('例如：苏州大学')
        self.buyer_expected_edit.setText(config.get('buyer_expected_name', '苏州大学'))
        ...
        list_display_label = QLabel('列表显示：')
        self.show_tags_checkbox = QCheckBox('显示标签')
        self.show_tags_checkbox.setChecked(config.get('list_show_tags', True))
        self.show_validation_checkbox = QCheckBox('显示验证状态')
        ...
        # 保存时显示提示
        self.show_save_prompt_checkbox = QCheckBox('保存数据时显示提示')
        ...
        # 删除前确认
        self.confirm_delete_checkbox = QCheckBox('删除发票前确认')
    ...
    def test_tesseract_path(self):
        ...
        if not path:
            QMessageBox.warning(self, '提示', '请输入 Tesseract 路径。')
            return
        ...
        if not path.lower().endswith('tesseract.exe'):
            QMessageBox.warning(self, '路径无效', '请选择 tesseract.exe 文件。')
            return
...
class TagsDialog(QDialog):
    ...
    def __init__(self, parent=None, current_tags=None):
        ...
        self.custom_tag_input.setPlaceholderText('输入自定义标签名称...')
        self.custom_tag_input.returnPressed.connect(self.add_custom_tag)
        custom_layout.addWidget(QLabel('自定义标签：'))
...
版本：2.4.3
开发者：石殷睿
所属单位：苏州大学（Soochow University）
专业：22EE（电子信息工程）

功能简介：
本软件用于辅助整理和初步识别发票、账单信息，方便进行分类、统计与简单核对。
支持PDF发票导入、OCR识别、商品明细提取、购买记录匹配等功能。
        """.strip())
...
个人主页：https://quartzsyr.github.io/photograph/
        """.strip())
...
图表绘制：matplotlib
        ...
        tab_widget.addTab(dev_tab, '开发者信息')
...
1. 本软件仅对票据/账单信息进行初步识别和整理，识别结果可能存在错误、遗漏或不完整的情况。

2. 请务必对软件输出结果进行人工复核，不要将识别结果作为唯一依据进行财务或法律决策。

3. 如遇到明显异常结果，建议重新导入数据或联系作者反馈。

4. 软件可能出错，仅供参考，请务必注意辨别结果。
        """.strip())
...
1. 本软件以"现状"提供，不对功能的准确性、完整性或适用性作出任何明示或默示担保。

2. 因使用或无法使用本软件所产生的任何直接或间接损失，由使用者自行承担，作者不对由此造成的后果承担任何责任。

3. 安装和使用本软件即视为您已充分理解并同意本免责声明的全部内容。

4. 本软件仅供学习和研究使用，不得用于商业用途或任何可能产生法律后果的场景。

5. 作者保留随时修改、更新或终止本软件的权利，无需事先通知。

6. 如因使用本软件而产生的任何纠纷，使用者应自行承担全部责任。
        """.strip())
...
本软件及其源代码受版权法保护。未经授权，不得复制、修改、分发或用于商业目的。

开源许可：本软件采用 MIT 许可证，允许自由使用、修改和分发，但需保留版权声明。
        """.strip())
        copyright_layout.addWidget(copyright_text)
        disclaimer_layout.addWidget(copyright_group)
        
        tab_widget.addTab(disclaimer_tab, '免责声明')
...
class InvoiceManager(QMainWindow):
    ...
    def init_ui(self):
        """初始化用户界面"""
        self.setWindowTitle('发票自动管理分类系统')
        ...
        self.nav_btn_home = QPushButton('发票管理')
        ...
        self.nav_btn_stats = QPushButton('统计分析')
        ...
        self.count_label = QLabel('共 0 张发票')
        ...
        # 购买记录匹配按钮：批量 OCR 识别实付金额并与发票匹配后重命名
        self.match_records_btn = QPushButton('匹配购买记录并重命名')
        ...
        left_layout.addWidget(QLabel('发票列表'))
        ...
        self.show_in_explorer_btn = QPushButton('📁 在文件夹中显示')
        ...
        self.relink_file_btn = QPushButton('🔗 重新关联')
        ...
        self.copy_file_btn = QPushButton('📦 复制文件')
        ...
        self.manage_tags_btn = QPushButton('🏷️ 管理标签')
        ...
        self.validate_btn = QPushButton('✓ 验证发票')
        ...
        # 商品明细表格
        table_group = QGroupBox('商品明细')
        table_layout = QVBoxLayout(table_group)

        self.items_table = QTableWidget()
        self.items_table.setColumnCount(6)
        self.items_table.setHorizontalHeaderLabels(['分类', '商品名称', '数量', '单价', '金额', '规格'])
        # 表格外观优化
        ...
        self.edit_category_btn = QPushButton('修改分类')
        ...
        self.delete_item_btn = QPushButton('删除商品')
        ...
        # 将商品明细表格添加到布局，并设置拉伸因子，确保它能占用剩余空间
        right_layout.addWidget(table_group, stretch=2)  # 给商品明细更大的权重，确保可见
        ...
        # ========== 按分类统计 ==========
        stats_category_group = QGroupBox('按商品分类统计')
        ...
        self.stats_category_table.setHorizontalHeaderLabels(['分类', '商品数量', '合计金额', '占比'])
        ...
        # ========== 按日期统计 ==========
        stats_date_group = QGroupBox('按日期统计')
        date_layout = QVBoxLayout(stats_date_group)
        
        # 日期统计选项
        date_options_layout = QHBoxLayout()
        date_options_layout.addWidget(QLabel('统计维度：'))
        self.date_stats_type_combo = QComboBox()
        self.date_stats_type_combo.addItem('按月统计', 'month')
        self.date_stats_type_combo.addItem('按年统计', 'year')
        self.date_stats_type_combo.addItem('按日统计', 'day')
        ...
        self.stats_date_table.setHorizontalHeaderLabels(['日期', '发票数量', '合计金额', '占比'])
        ...
        # ========== 验证状态统计 ==========
        stats_validation_group = QGroupBox('验证状态统计')
        ...
        self.stats_validation_table.setHorizontalHeaderLabels(['验证状态', '发票数量', '合计金额'])
    ...
    def update_pie_chart(self, canvas, data_dict, title, total_amount):
        ...
        if len(sorted_items) > 10:
            ...
            if other_amount > 0:
                top_items.append(('其他', {'amount': other_amount, 'count': other_count}))
    ...
    def update_bar_chart(self, canvas, data_dict, title, xlabel='日期', ylabel='金额（元）'):
        """更新柱状图"""
        if not data_dict:
            canvas.figure.clear()
            ax = canvas.figure.add_subplot(111)
        ...
        if len(sorted_items) > 20:
            sorted_items = sorted_items[-20:]  # 只显示最近20个
    ...
    def import_invoices(self):
        ...
        self.status_bar.showMessage('正在处理发票...')
    ...
    def on_import_finished(self, invoices):
        ...
        for inv in invoices:
            ...
            if validation_result['errors']:
                validation_errors.append({
                    'invoice': inv.get('invoice_number', '未知'),
        ...
        # 构建提示信息
        msg_parts = [f'成功导入 {added_count} 张发票']
        
        if duplicate_count > 0:
            msg_parts.append(f'跳过重复发票 {duplicate_count} 张')
        
        if validation_errors:
            error_count = len(validation_errors)
            msg_parts.append(f'发现 {error_count} 张发票有错误')
        
        if validation_warnings:
            warning_count = len(validation_warnings)
            msg_parts.append(f'发现 {warning_count} 张发票有警告')
        
        self.status_bar.showMessage(' | '.join(msg_parts))
        
        # 显示详细验证结果
        if validation_errors or validation_warnings or duplicate_count > 0:
            detail_msg = f'成功导入 {added_count} 张发票。\n\n'
            
            if duplicate_count > 0:
                detail_msg += f'跳过重复发票：{duplicate_count} 张\n'
            
            if validation_errors:
                detail_msg += f'\n发现 {len(validation_errors)} 张发票有错误：\n'
                for item in validation_errors[:5]:  # 最多显示5个
                    detail_msg += f"  • {item['invoice']}: {', '.join(item['errors'][:2])}\n"
                if len(validation_errors) > 5:
                    detail_msg += f"  ... 还有 {len(validation_errors) - 5} 张发票有错误\n"
            
            if validation_warnings:
                detail_msg += f'\n发现 {len(validation_warnings)} 张发票有警告（可忽略）：\n'
                for item in validation_warnings[:3]:  # 最多显示3个
                    detail_msg += f"  • {item['invoice']}: {', '.join(item['warnings'][:1])}\n"
                if len(validation_warnings) > 3:
                    detail_msg += f"  ... 还有 {len(validation_warnings) - 3} 张发票有警告\n"
            
            detail_msg += '\n提示：可以在发票详情中查看完整的验证信息。'
            
            QMessageBox.information(self, '导入完成', detail_msg)
        else:
            QMessageBox.information(self, '导入完成', f'成功导入 {added_count} 张发票，所有发票验证通过。')

    def on_import_error(self, error_msg):
        """导入错误"""
        QMessageBox.warning(self, '导入错误', error_msg)

    # ===== 顶部导航交互（当前只是样式和选中态，后续可扩展为多页面） =====
    def on_top_nav_clicked(self, clicked_btn):
        """顶部导航按钮点击时，高亮当前选项（预留多页面切换入口）"""
        # 保证导航按钮互斥选中
    ...
    def update_stats_view(self):
        ...
        if not self.invoices:
            # 清空所有统计表格
            self.stats_summary_label.setText('共 0 张发票，合计金额 ¥0.00')
        ...
        date_stats = {}
        tag_stats = {}
        validation_stats = {'通过': {'count': 0, 'amount': 0.0}, 
                          '有错误': {'count': 0, 'amount': 0.0},
                          '有警告': {'count': 0, 'amount': 0.0},
                          '未验证': {'count': 0, 'amount': 0.0}}
        ...
        for inv in self.invoices:
            ...
            if date_str:
                try:
                    ...
                    if '年' in date_str and '月' in date_str:
                        # 格式：2025年12月02日
                        parts = date_str.replace('日', '').split('年')
                        if len(parts) == 2:
                            year = parts[0]
            ...
            if validation:
                if not validation['is_valid']:
                    validation_stats['有错误']['count'] += 1
                    validation_stats['有错误']['amount'] += amount
                elif validation['warnings']:
                    validation_stats['有警告']['count'] += 1
                    validation_stats['有警告']['amount'] += amount
                else:
                    validation_stats['通过']['count'] += 1
                    validation_stats['通过']['amount'] += amount
            else:
                validation_stats['未验证']['count'] += 1
                validation_stats['未验证']['amount'] += amount
        ...
        self.stats_summary_label.setText(
            f'共 {total_count} 张发票，合计金额 ¥{total_amount:.2f}'
        )
        
        # 更新详细统计信息
        detail_text = f"""
平均金额：¥{avg_amount:.2f}  |  最大金额：¥{max_amount:.2f}  |  最小金额：¥{min_amount:.2f}
        """
        ...
        for row, (seller, data) in enumerate(sorted_sellers):
            ...
            self.stats_seller_table.setItem(row, 2, QTableWidgetItem(f"¥{data['amount']:.2f}"))
        ...
        if hasattr(self, 'seller_chart_canvas'):
            self.update_pie_chart(self.seller_chart_canvas, seller_stats, '按销售方金额分布', total_amount)
        ...
        for row, (category, data) in enumerate(sorted_categories):
            ...
            self.stats_category_table.setItem(row, 2, QTableWidgetItem(f"¥{data['amount']:.2f}"))
            percentage = (data['amount'] / total_amount * 100) if total_amount > 0 else 0
        ...
        if hasattr(self, 'category_chart_canvas'):
            ...
            self.update_pie_chart(self.category_chart_canvas, category_stats, '按商品分类金额分布', category_total)
        ...
        for row, (date_key, data) in enumerate(sorted_dates):
            ...
            self.stats_date_table.setItem(row, 2, QTableWidgetItem(f"¥{data['amount']:.2f}"))
            percentage = (data['amount'] / total_amount * 100) if total_amount > 0 else 0
            self.stats_date_table.setItem(row, 3, QTableWidgetItem(f"{percentage:.1f}%"))
        
        # 更新日期趋势柱状图
        if hasattr(self, 'date_chart_canvas'):
            date_type_name = {'month': '月度', 'year': '年度', 'day': '日期'}.get(date_stats_type, '日期')
            self.update_bar_chart(self.date_chart_canvas, date_stats, f'{date_type_name}金额趋势', '日期', '金额（元）')
        ...
        for row, (tag, data) in enumerate(sorted_tags):
            ...
            self.stats_tag_table.setItem(row, 2, QTableWidgetItem(f"¥{data['amount']:.2f}"))
            percentage = (data['amount'] / total_amount * 100) if total_amount > 0 else 0
        ...
        for status, data in validation_stats.items():
            if data['count'] > 0:
                ...
                self.stats_validation_table.setItem(row, 2, QTableWidgetItem(f"¥{data['amount']:.2f}"))
                row += 1
    ...
    def update_invoice_list(self):
        ...
        for invoice in self.invoices:
            ...
            item_text = f"{file_name} - {seller} - ¥{amount:.2f}{tags_str}{validation_mark}"
            ...
            if validation and not validation['is_valid']:
                ...
                item.setForeground(QBrush(QColor(200, 0, 0)))  # 深红色文字
            # 如果有标签，设置不同的颜色
            elif tags:
                # 根据标签设置颜色（优先级：已有支付记录 > 已报销 > 待审核 > 重要 > 其他）
                if '已有支付记录' in tags:
                    item.setForeground(QBrush(QColor(34, 139, 34)))  # 绿色
                elif '已报销' in tags:
                    item.setForeground(QBrush(QColor(30, 144, 255)))  # 蓝色
                elif '待审核' in tags:
                    item.setForeground(QBrush(QColor(255, 140, 0)))  # 橙色
                elif '重要' in tags:
                    item.setForeground(QBrush(QColor(220, 20, 60)))  # 红色
                else:
                    item.setForeground(QBrush(QColor(138, 43, 226)))  # 紫色
    ...
    def display_invoice_details(self, invoice):
        ...
        if validation:
            if validation['is_valid']:
                validation_status = '✓ 验证通过'
                if validation['warnings']:
                    validation_status += f'（{len(validation["warnings"])} 个警告）'
            else:
                validation_status = f'✗ 验证失败（{len(validation["errors"])} 个错误'
                if validation['warnings']:
                    validation_status += f'，{len(validation["warnings"])} 个警告'
                validation_status += '）'
        else:
        ...
        details = f"""
发票号码: {invoice.get('invoice_number', '未知')}
开票日期: {invoice.get('date', '未知')}
销售方: {invoice.get('seller', '未知')}
购买方: {invoice.get('buyer', '未知')}
总金额: ¥{invoice.get('total_amount', 0):.2f}
标签: {tags_text}
...
文件状态: {file_status}
    ...
    def edit_category(self):
        ...
        if current_row >= 0:
            categories = ['耗材', '设备', '其他']
            category, ok = QInputDialog.getItem(self, '选择分类', '分类:', categories, 0, False)
            if ok:
    ...
    def add_item(self):
        ...
        if current_invoice_index >= 0:
            ...
            # 默认值
            self.items_table.setItem(row_count, 0, QTableWidgetItem('其他'))
            self.items_table.setItem(row_count, 1, QTableWidgetItem('新商品'))
    ...
    def export_bill(self):
        """导出账单"""
        if not self.invoices:
            QMessageBox.warning(self, '警告', '没有发票数据可导出')
            return
        ...
        if default_path and os.path.exists(default_path):
            default_file = os.path.join(default_path, '账单.xlsx')
        else:
            default_file = '账单.xlsx'
        
        file_path, _ = QFileDialog.getSaveFileName(
            self, '保存账单', default_file, 'Excel文件 (*.xlsx);;所有文件 (*.*)'
        )
        ...
        try:
            ...
            QMessageBox.information(self, '成功', f'账单已保存到: {file_path}')
        except Exception as e:
    ...
    def generate_bill_excel(self, file_path):
        """生成账单Excel文件"""
        bill_data = []
        # total_amount 按每张发票的「价税合计」求和（含税），
        # 而不是按明细的未税金额累加，避免漏算税额。
        total_amount = 0.0
        ...
        for invoice in self.invoices:
            ...
            for item in items:
                bill_data.append({
                    '一级产品分类*': item.get('category', '其他'),
                    '商品名称*': item.get('name', ''),
                    '单价（元）*': item.get('price', 0),
                    '数量*': item.get('quantity', 1),
                    '包装单位': item.get('unit', '个'),
                    '品 牌': '',
                    '货号': '',
                    '规格': item.get('spec', '')
                })
    ...
    def clear_data(self):
        ...
        # 执行清空
            ...
            self.export_btn.setEnabled(False)
            self.total_label.setText('总金额：¥0.00')
            if hasattr(self, 'count_label'):
                self.count_label.setText('共 0 张发票')
            # 清空统计视图
            self.update_stats_view()
    ...
    def save_invoices(self):
        """将当前发票数据自动保存到本地 JSON 文件"""
        autosave_path = self._get_autosave_path()
        try:
            with open(autosave_path, 'w', encoding='utf-8') as f:
                # self.invoices 结构只包含基本类型（字符串、数字、列表、字典），可直接 JSON 序列化
                json.dump(self.invoices, f, ensure_ascii=False, indent=2)
        except Exception as e:
            # 自动保存失败不打断用户操作，仅在控制台输出
            print(f'自动保存发票数据失败: {e}')
    ...
    def load_invoices(self):
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                self.export_btn.setEnabled(bool(self.invoices))
                if self.invoices:
                    self.status_bar.showMessage(f'已自动加载上次的 {len(self.invoices)} 张发票数据')
        except Exception as e:
            print(f'自动加载发票数据失败: {e}')
    ...
    def open_settings(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            config = load_config()
            # 合并新设置
            config.update(dialog.get_settings())
            if save_config(config):
                QMessageBox.information(
                    self,
                    '设置已保存',
                    '设置已保存。\n\n下次使用 OCR / 匹配功能时将使用新的配置。'
                )
            else:
                QMessageBox.warning(self, '保存失败', '无法保存配置文件，请检查文件权限。')
    ...
    def open_invoice_file(self):
        ...
        file_path = invoice.get('file_path', '')
        if not file_path:
            QMessageBox.warning(self, '错误', '该发票没有关联文件路径')
            return
        
        if not os.path.exists(file_path):
            QMessageBox.warning(self, '文件不存在', f'找不到文件：\n{file_path}\n\n请使用"重新关联"功能指定正确的文件路径。')
            return
        
        try:
            # Windows 系统使用 os.startfile，跨平台可以使用 subprocess
    ...
    def show_file_in_explorer(self):
        ...
        try:
            ...
            QMessageBox.critical(self, '操作失败', f'无法打开文件夹：\n{str(e)}')
    ...
    def copy_file_path(self):
        ...
        try:
            ...
            QMessageBox.information(self, '已复制', f'文件路径已复制到剪贴板：\n{file_path}')
        except Exception as e:
            QMessageBox.critical(self, '复制失败', f'无法复制路径：\n{str(e)}')

    def relink_invoice_file(self):
        """重新关联发票文件（如果文件移动或重命名了）"""
        invoice = self.get_current_invoice()
        if not invoice:
            QMessageBox.warning(self, '提示', '请先选择一张发票')
            return
        ...
        QMessageBox.information(self, '成功', '发票文件已重新关联')
    ...
    def copy_invoice_file(self):
        ...
        if not file_path or not os.path.exists(file_path):
            QMessageBox.warning(self, '文件不存在', '找不到发票文件，无法复制')
            return
        ...
        try:
            ...
            QMessageBox.information(self, '成功', f'文件已复制到：\n{target_path}')
        except Exception as e:
            QMessageBox.critical(self, '复制失败', f'无法复制文件：\n{str(e)}')
    ...
    def manage_invoice_tags(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            QMessageBox.information(self, '成功', '标签已更新')
    ...
    def show_validation_result(self, invoice, validation_result):
        """显示验证结果对话框"""
        invoice_number = invoice.get('invoice_number', '未知')
        
        if validation_result['is_valid'] and not validation_result['warnings']:
            QMessageBox.information(
                self,
                '验证通过',
                f'发票 {invoice_number} 验证通过，所有检查项均正常。'
            )
        else:
            msg = f'发票 {invoice_number} 验证结果：\n\n'
            ...
            msg += f'\n验证时间：{validation_result.get("checked_at", "未知")}'
            
            if validation_result['is_valid']:
                QMessageBox.warning(self, '验证完成（有警告）', msg)
            else:
    ...
    def match_purchase_records(self):
        """
        批量从购买记录图片中 OCR 识别“实付金额”，
        与当前发票的总金额进行匹配，并按
        「发票号_金额_原文件名」格式重命名图片。
        """
        # 仅在用户点击「匹配购买记录并重命名」时，才初始化/检测 Tesseract
        if not init_tesseract_or_warn(self):
            return
        if not self.invoices:
            QMessageBox.warning(self, '提示', '请先导入并解析发票，再进行购买记录匹配。')
            return

        folder = QFileDialog.getExistingDirectory(self, '选择购买记录图片所在文件夹', '')
        if not folder:
            return
        ...
        if not files:
            QMessageBox.information(self, '提示', '所选文件夹中没有找到支持的图片文件（jpg/png等）。')
            return

        # 使用后台线程执行 OCR 匹配，避免阻塞界面
        self.status_bar.showMessage('正在匹配购买记录，请稍候...')
    ...
    def on_match_finished(self, result):
        ...
        msg_lines = [
            f"共扫描购买记录图片：{total} 个",
            f"成功匹配并重命名：{matched_count} 个",
        ]
        if unmatched_files:
            msg_lines.append(f"未找到匹配发票：{len(unmatched_files)} 个")
        if error_files:
            msg_lines.append(f"OCR或重命名出错：{len(error_files)} 个")

        detail_text = ""
        if unmatched_files:
            detail_text += "未匹配文件示例：\n" + "\n".join(unmatched_files[:10])
            if len(unmatched_files) > 10:
                detail_text += f"\n…… 等 {len(unmatched_files)} 个\n"
        if error_files:
            if detail_text:
                detail_text += "\n"
            detail_text += "出错文件示例：\n" + "\n".join(error_files[:10])
        ...
        self.status_bar.showMessage('购买记录匹配完成。')
    ...
    def on_match_error(self, error_msg):
        ...
        self.progress_bar.setVisible(False)
        self.match_records_btn.setEnabled(True)
        self.status_bar.showMessage('购买记录匹配出错。')
        QMessageBox.warning(self, '匹配错误', error_msg)


    def _extract_paid_amount_from_image(self, img_path):
        """
        使用 OCR 从购买记录图片中提取“实付金额”。
        优先匹配包含“实付/实收/实付金额/支付金额”等关键词的行上的金额；
        若未找到，再从整段文本中取最大金额作为候选。
        """
        try:
            image = Image.open(img_path)
        except Exception as e:
        ...
        # 1. 优先在含“实付/实收/实付金额/支付金额”等关键词的行里找金额
        keywords = ['实付', '实收', '实付金额', '支付金额', '应付金额', '合计', '总计']
        candidate_amounts = []
        ...
        if candidate_amounts:
            # 取该行中最大的一个金额作为「实付金额」候选
            return max(candidate_amounts)

        # 2. 如果关键词行找不到，再从整段文本中取最大的金额
        all_amounts = []
        for line in lines:
            for m in amount_pattern.finditer(line.replace(',', '')):
                try:
    ...
    def _find_invoice_by_amount(self, amount, tol=0.01):
        """
        根据实付金额在当前发票列表中查找匹配发票。
        若存在多张金额相同的发票，这里简单返回第一张，避免自动做出含糊匹配。
        """
        if amount is None:
            return None
...
def main():
    # 修复Qt平台插件路径问题（必须在创建QApplication之前设置）
    if getattr(sys, 'frozen', False):
        # 如果是打包后的exe
    ...
    # 设置Qt插件路径（使用环境变量和QCoreApplication两种方法确保兼容性）
    plugin_path = os.path.join(base_path, 'PyQt5', 'Qt5', 'plugins')
    if os.path.exists(plugin_path):
        os.environ['QT_PLUGIN_PATH'] = plugin_path
        # 使用QCoreApplication的方法（需要在创建QApplication之前）
        QCoreApplication.setLibraryPaths([plugin_path])
    
    # pdfium路径已在文件开头设置，这里不需要重复设置
    
    app = QApplication(sys.argv)
    app.setStyle('Fusion')

    # 全局现代化样式（浅色主题，参考主流前端设计语言：Ant Design / Material）
    app.setStyleSheet("""
        /* 全局基础字体与背景 */
        QWidget {
        ...
        QPushButton#topNavButton:checked {
            ...
            padding-bottom: 4px; /* 边框占用 2px，整体高度保持一致 */
        }
        ...
        /* 幽灵按钮（ghost）：无背景，仅边框 + 文本色变化，适合次要操作 */
    ...
    # 设置应用程序信息
    app.setApplicationName('发票自动管理分类系统')
